...
2. `/ponytail full` — disciplina de engenharia: YAGNI, stdlib/nativo antes de dependência, mudanças cirúrgicas, execução orientada a meta verificável.
3. `karpathy-guidelines` — obrigatória durante todo processo de implementação e revisão técnica: pensar antes de codar, simplicidade, mudanças cirúrgicas, execução orientada a meta verificável.
